...
const Login = ({ handleLogin }) => {
  ...
  const submitHandler = async (e) => {
    e.preventDefault()
    setLoading(true)
    // Small delay so the loading state is visible — feels more intentional
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(135deg, #e8e4f8 0%, #f0f7ee 50%, #fde8e8 100%)' }}
    >
      ...
      <div className="relative z-10 bg-white rounded-3xl shadow-xl px-10 py-10 w-[440px] transition-all duration-300">
        ...
        <p className="text-gray-400 text-sm mb-8">Sign in to pick up where your team left off.</p>
        ...
        <form onSubmit={submitHandler} className="flex flex-col gap-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-full text-white font-semibold text-sm mt-2
        ...
        <p className="text-center text-sm text-gray-400 mt-6">
          ...
          <span className="text-purple-500 font-medium cursor-pointer hover:text-purple-400 transition-colors duration-200">
            Sign up
          </span>
        </p>
